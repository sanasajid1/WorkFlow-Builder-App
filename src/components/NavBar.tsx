...
export const NavBar: React.FC = () => {
  ...
  return (
    <TabGroup as="div" className="h-full flex flex-col">
      <div className="flex font-inter items-center justify-between border-b border-borderGray200 px-4 py-2 bg-white">
        ...
        <TabList className="flex space-x-4">
          {tabs.map((tab) => (
            <Tab
              key={tab.name}
              className={({ selected }) =>
                `text-base font-medium px-2 pb-2 focus:outline-none focus:ring-0 flex items-center gap-2 ${
                  selected
      ...
      <TabPanels className="flex-1 p-4">
        <TabPanel>
          <div className="text-lg">Hello, this is Builder</div>
        </TabPanel>
        <TabPanel>
